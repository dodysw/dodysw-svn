...
def process_data(sock, line):
    ...
    for comm_group in line.split('|'):
        ...
        if cmd == '$Lock':
            ...
            if (options.pm_only and cmd == '$To:' and 'showmecs' in comm_group) or (not options.pm_only and 'showmecs' in comm_group):
                # findout the source
                ...
                if m:
                    ...
                    if __debug__: print 'Send', data
                    sock.send(data + '|')
            elif options.pm_only and cmd != '$To:' and 'showmecs' in comm_group:
                # findout the source
                m = re.search('<([^>]*)>',comm_group)
                if m:
                    msg = 'pycsdcbot is configured to reply only via personal message. Type this instead: /pm %s showmecs' % nick
...
if __name__ == '__main__':
    ...
    parser.add_option("--nick", dest="nick", help="DC++ nick name (def:cs_bot)", default='cs_bot')
