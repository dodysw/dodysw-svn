...
to include DotA games for ANU's fennetic server
"""
...
__author__ = 'Dody Suria Wijaya <dodysw@gmail.com>. modifided by Mark Mckay'
__email__ = 'dodysw@gmail.com'

# -- pydcbot config
nick = 'Game_bot'
password = 'game_bot_cs/Dota'
description = 'Game server bot'
tag = '<pyGameDcBot++ V:%s,M:A,H:1/0/0,S:1>' % __version__
connection_type = 'LAN(T3)'
email = ''
sharesize = 1
state = 0
## added in some new triggers
trigger_word = ['showmecs', 'dotaserver', 'dotaplayer', 'help']
reply_public_channel = False    # True to reply trigger_word from public channel, in addition to private channel
## and a new array hope these are right
DotAservers = []
# --
...
def process_data(sock, line):
    ...
    for comm_group in line.split('|'):
        ...
        if cmd == '$Lock':
            ...
            if (options.pm_only and cmd == '$To:' and trigger_word[0] in comm_group) or (not options.pm_only and trigger_word[0] in comm_group):
                # findout the source
                m = re.search('<([^>]*)>',comm_group)
                if m:
                    ## addition from here
                    #check trigger words
                    if trigger_word[1] in comm_group:
                        mip = re.search('[12]?[0-9]?[0-9](\.[12]?[0-9]?[0-9]){3}',comm_group)
                        if mip:
                            if m.group(1) in DotAservers:
                                DotAservers.remove('Host: %s, IP: %s ' % (m.group(1),mip.group(1))) ## this right?
                            else:
                                if DotAservers[0] == '':
                                    DotAservers[0] = 'Host: %s, IP: %s ' % (m.group(1),mip.group(1))
                                else:
                                    DotAservers.append('Host: %s, IP: %s ' % (m.group(1),mip.group(1)))
                                message_players
                        else:
                            msg = 'no ip given please re pm with the server ip'
                            data = '$To: %s From: %s $<%s> %s' % (m.group(1), nick, nick, msg)
                            sock.send(data + '|')
                    elif trigger_word[2] in comm_group:
                        if m.group(1) in DotAplayers:
                            DotAplayes.remove(m.group(1)) ## also here
                        else:
                            if DotAplayers[0] == '': DotAplayers[0] = m.group(1)
                            else: DotAplayers.append(m.group(1))
                            for n in range(0,len(DotAservers)):
                                msg= msg + DotAservers[n] + '\n'
                                data = '$To: %s From: %s $<%s> %s' % (m.group(1), nick, nick, msg)
                                sock.send(data + '|')
                    elif trigger_word[3] in comm_group:
                        help = 'To view CS info msg showmecs. To register and unregister a dota server msg "dotaserver" and your ip. To register and unregister as a dota player msg dotaplayer.'
                        data = '$To: %s From: %s $<%s> %s' % (m.group(1), nick, nick, help)
                        sock.send(data + '|')
                    else:
                    ## addition ends
                        ...
                        if __debug__: print 'Send', data
                        sock.send(data + '|')
            elif options.pm_only and cmd != '$To:' and trigger_word[0] in comm_group:
                # findout the source
                m = re.search('<([^>]*)>',comm_group)
                if reply_public_channel and m:
                    msg = 'pygamedcbot is configured to reply only via personal message. Type this instead: /pm %s help' % (nick)
                    data = '$To: %s From: %s $<%s> %s' % (m.group(1), nick, nick, msg)
                    if __debug__: print 'Send', data
                    sock.send(data + '|')
            ## addition from here
            elif 'dota' in comm_group:
                m = re.search('<([^>]*)>',comm_group)
                if m:
                    mip = re.search('[12]?[0-9]?[0-9](\.[12]?[0-9]?[0-9]){3}',comm_group)
                    if mip:
                        if m.group(1) not in DotAservers:
                            if DotAservers[0]=='': DotAservers[0] = 'Host: %s, IP: %s ' % (m.group(1),mip.group(1))
                            else: DotAservers.append('Host: %s, IP: %s ' % (m.group(1),mip.group(1)))
                        Message_players
            ## addition ends
            elif comm_group[0] == '<':
...
## new def to make things easier
def Message_players():
    ## to message players when a server is added
    for i in range(0,len(DotAplayers)):
        msg = 'current servers: \n'
        for n in range(0,len(DotAservers)):
            msg= msg + DotAservers[n] + '\n'
        data = '$To: %s From: %s $<%s> %s' % (DotAplayesr[i], nick, nick, msg)
        sock.send(data + '|')
...
## this was in the python manual for converting to lower so maybe you could try that
#~ Py_UNICODE_TOLOWER(Py UNICODE ch)