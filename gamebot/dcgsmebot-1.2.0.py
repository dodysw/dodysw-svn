...
sharesize = 1
state = 0
T_SHOWMECS, T_SHOWMEDOD, T_DOTASERVER, T_DOTAPLAYER, T_HELP, T_SHOWMEDOTA, T_PUBLICDOTA= trigger_word = ['showmecs', 'showmedod', 'dotaserver', 'dotaplayer', 'help', 'showmedota', 'dota']
dota_proper = {}
dota_players = {}
cs_players = {}
regip='[12]?[0-9]?[0-9](\.[12]?[0-9]?[0-9]){3}'

def check_cs_source(addr, port):
...
def trigger(cmd,comm,sock):
    ...
    if cmd == '$To:':
        ...
        if T_SHOWMECS in comm:
            ret = check_cs_source(options.cs_server_ip, options.cs_server_port)
            if ret == False:
                ...
                t1,t2,mapname = ret
                dlg_view_msg = '\n\n%s of %s people. Map: %s. Server: %s (%s:%s)\n%s' % (t1,t2,mapname, server_name, server_addr, server_port, misc)
        ...
        elif T_SHOWMEDOD in comm:
            ret = check_cs_source(options.dod_server_ip, options.dod_server_port)
            if ret == False:
                simple_msg = 'Unable to connect to %s:%s' % (options.cs_server_ip, options.cs_server_port)
                data = '$To: %s From: %s $<%s> %s - %s' % (from_nick, nick, nick, simple_msg, random.random())
            else:
...
Available commands (without quotes):
    "showmecs" = view counter strike source players and scores.
    "showmedod" = view day of defeat players and scores.
...
if __name__ == '__main__':
    ...
    parser.add_option("--dod_ip", dest="dod_server_ip", help="DOD Server address (def:150.203.239.103)", default='150.203.239.103')
    parser.add_option("--dod_port", type="int", dest="dod_server_port", help="DOD Server port (def:27018)", default=27018)
    ...
    nick, dc_server_ip, dc_server_port, cs_server_ip, cs_server_port, dod_server_ip, dod_server_port, dlg_view_msg = options.nick, options.dc_server_ip, options.dc_server_port, options.cs_server_ip, options.cs_server_port, options.dod_server_ip, options.dod_server_port,''
