import urllib, re, email,email.MIMEText,datetime
...
class HTTPMaildrop:
    ...
    def do_detik_getindex(self):
        ...
        today = datetime.date.today()
        thn,bln,tgl = today.year,today.month,today.day
        url = 'http://www.detik.com/index.htm'
        buffer = urllib.urlopen(url).read()
        ...
        for atag in parser.anchorlist:
            if "%s/%02d/%02d" % (thn,bln,tgl) in atag:
                if atag[0] == '/': atag = 'http://www.detik.com'+atag
                #in case url is like: http://www.detik.com/peristiwa/index.php?url=http://www.detik.com/peristiwa/2004/01/26/20040126-150358.shtml
                if atag.count('http') > 1: atag = atag[atag.rfind('http'):]
                atags.append(atag)
            # from 29 mar 04 - detik change some news url to like "http://www.detiknews.com/index.php/detik.read/tahun/2004/bulan/04/tgl/02/time/91725/idnews/123085/idkanal/10"
            elif "www.detiknews.com" in atag:
                atags.append(atag)
        parser.close()
        all_urls.extend(atags)
        #dedupe urls
    ...
    def send_msg(self,msg_no,linecount=-1):
        ...
        if msg == "":
            ...
            msg = parsenews2email(newsdict).replace("\n","\r\n")
...
def parsenews(buffer, url=None):
    ...
    if len(buffer)<100:
        ...
        return dict(subjudul=subjudul,judul=judul,reporter=reporter,berita='news length not make sense')
    regex = 'Kode kesalahannya <strong>404</strong>'
    m = re.search(regex,buffer,re.DOTALL|re.IGNORECASE)
    if m:
        return dict(subjudul=subjudul,judul=judul,reporter=reporter,berita="Kode kesalahan 404")
    while 1:
        ...
        berita = "\r\n--------BUFFER----\r\n%s\r\n-------END BUFFER-----------\r\n" % buffer
        return dict(subjudul=subjudul,judul=judul,reporter=reporter,berita=berita)

    if berita.strip() == "":
        if debug: print "news zero content",url
        return dict(subjudul=subjudul,judul=judul,reporter=reporter,berita="news zero content")
    if judul.strip() == "":
        if debug: print "title zero content",url
        return dict(subjudul=subjudul,judul=judul,reporter=reporter,berita="title zero content")

    return dict(subjudul=subjudul,judul=judul,reporter=reporter,berita=berita)

def parsenews2email(newsdict):
...
---------------------------------------------------
%(url)s
by popok w/ detikcom module => http://dsw.gesit.com
---------------------------------------------------
""" % newsdict
    mailmsg = email.MIMEText.MIMEText(body)
    ...
    mailmsg.add_header('X-Mailer','popok - webmail to pop3/smtp gateway - dswsh@plasa.com')
