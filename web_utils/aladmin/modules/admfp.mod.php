...
class admfp {
    function admfp() {
        ...
        $this->navbar = 0;
        $this->max_col_num = 4; # number of column on welcome page
    }

    function go() {
        global $appconf;
        // called inside main content
        echo '<div align=center><h1>'.$appconf['site_title'].'</h1><p><i>'.$appconf['site_description'].'</i><p>';
        global $navdata;
        $all = array();
        $total_vlen = 0;
        ...
        $count_switch = floor($total_vlen / $this->max_col_num);
        $curr_count = 0;
        $switch_now = 0;
