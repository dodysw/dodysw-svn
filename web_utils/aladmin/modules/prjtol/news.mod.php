...
class news extends TableManager {
    ...
    function news() {
        ...
        $this->properties['title'] = new Prop(array('label'=>'Title','colname'=>'title','required'=>True,'length'=>100));
    ...
    function fe_list($maxrow,$cat_id='*') {
        ...
        for ($i = 0; $i < $this->db_count; $i++) {
            ...
            echo "<p><b><a href='index.php?p=news_view&id={$this->ds->_rowid[$i]}'>{$this->ds->title[$i]}</a></b>
            <small>({$this->ds->author[$i]} - {$this->ds->create_date[$i]} - $catdesc)</small> ";
    ...
    function construct_view($rowid) {
        ...
        $buffer .= '<p><b>('.$this->ds->create_date[$i] . ")</b> ".nl2br($this->ds->body[$i]).'<small>('.$this->ds->author[$i].')</small></p>';
        return $buffer;
    }
