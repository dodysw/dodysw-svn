...
/* functions used for navigation bar/tree */
...
function sn2($m,$title) {
    echo '<td nowrap="" id="Menu1" class="menubar" onclick="lg.ShowTpMn(\'Menu1\',\'Menu10\',false);event.cancelBubble = \'true\';" onmouseover="lg.MnOver(\'Menu1\',\'Menu10\',false);" onmouseout="lg.MnOut(\'Menu1\',\'Menu10\');">'.$title.'</td>';
}
...
function secure_hash($str) {
    /* create a hash for making sure URL parameter is not being tempered */
    $pass_hash = '192837hd91873hd'; # don't tell this to anyone, and change on every application
    return md5($pass_hash.$str);
}

function secure_hash_ok($input, $hash) {
    return (secure_hash($input) == $hash);
}
