...
class manage_modules extends TableManager {
    ...
    function manage_modules() {
        ...
        $this->enum_keyval = array('module_id','module_id');
        $this->unit = 'module';
    ...
    function act_merge_table($post) {
        ...
        for ($i = 0; $i < $num_cols; $i++) {
            ...
            $z_db[$colname]['flag'] = explode(' ',$flag);
            ...
            if ($z_fw[$colname]['is_key'] == True and !in_array('unique_key',$z_db[$colname]['flag'])) {
                $suggest_sql[] = "alter table `{$prog->db_table}` add unique (`{$colname}`);";
            }
            #~ if (in_array('unique_key',$z_db[$colname]['flag'] and $z_fw[$colname]['is_key'] == False) {
                #~ $suggest_sql[] = "alter table `{$prog->db_table}` add unique (`{$colname}`);";
            #~ }
        ...
        echo '<table border=1><tr><th>name</th><th>data type</th><th>Nullable</th><th>Unique</th></tr>';
        foreach ($z_db as $colname=>$val) {
            echo '<tr><td>'.$colname.'</td><td>'.$val['datatype'].'</td><td>&nbsp;'.(in_array('not_null',$val['flag'])?'':'Y').'</td><td>&nbsp;'.(in_array('unique_key',$val['flag'])?'Y':'').'</td></tr>';
        }
        echo '</table>';
