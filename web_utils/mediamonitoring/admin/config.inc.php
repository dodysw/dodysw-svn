...
/* global configuration
 copyright 2004 - dody suria wijaya, dsw software house - contact: dodysw@gmail.com */
    ...
    define('APP_PATH_ROOT',strtr(dirname(__FILE__),'\\','/'));  # decide automatically, override if not correct
    ...
    define('APP_JPGRAPH_ROOT',APP_INCLUDE_ROOT.'/jpgraph_php4');
	#~ define('LOG4PHP_DIR', APP_INCLUDE_ROOT.'/log4php'); #log4j...dir

    /* FORMATTING */
    $appconf['site_title'] = 'Al-Admin';
    ...
    $GLOBALS['server_protocol'] = (!empty($_SERVER['HTTPS']) && strtolower($_SERVER['HTTPS']) != 'off') ? 'https' : 'http';
    ...
    $GLOBALS['mail_server'] = 'anumail.anu.edu.au';  # comment this to use default smtp on hosting account
    $GLOBALS['mail_from'] = 'DoNotReply <invalid@email.com>';  # the email address used as from: header
    $GLOBALS['mail_replyto'] = 'DoNotReply <invalid@email.com>';  # the email address used as replyto: header, must be entered to avoid detected as spam
    $GLOBALS['mail_to'] = 'DummyTo <invalid@email.com>';  # the email address used as from: header
