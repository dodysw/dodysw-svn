...
$app['update_url'] = "http://miaw.tcom.ou.edu/~dody/detikusable-latest.php.txt";
$app['cache'] = TRUE;
...
//    FUNCTIONS
    ...
    function news_list_view(&$news)
    {
        ...
        {
            echo "<p><small>Boooo...topic news not available. ask <a href=mailto:dswsh@plasa.com>dody</a></small>";
        }
...
if ($detikusable_mode == 'news_detail') {
    ...
    else
    {
        ...
        $header_buffer = sock_recv_header ();

        //    recv all response body
        do
        {
           $data = fread($fp, 8192); #use fread as body may be binary (fget is not binary safe)
        ...
        {
            //skip the first 10 characters,as they are GZIP header, and php's gzinflate only need the data
            $buffers = gzinflate(substr($buffers, 10));
        }
...
if ($detikusable_mode == 'news_list')
{
    ...
    else
    {
        ...
        if (preg_match("|Content-Encoding:\s*gzip|i",$header_buffer))
        {
            //skip the first 10 characters,as they are GZIP header, and php's gzinflate only need the data
            #echo '>>'.$buffers.'<<';
            $buffers = gzinflate(substr($buffers, 10,-4));
        }
                ...
                else
                {
                    ...
                    for ($j = 0; $j < $count_news; $j++)
                    {
                        ...
                        else
                        {
                            ...
                            $news['topic'][$i]['news'][$j]['date'] = mktime($tgl[4],$tgl[5],0,$tgl[2],$tgl[3],$tgl[1]);
                        }
                    }
                }
            }
...
if ($detikusable_mode == 'auto_update')
{
    if (!$_REQUEST['commit'])
    {
        ...
        if ($buffers == ''){echo "<h1>update url:".$app['update_url']." was not found!</h1>";exit;}
        fclose($fp);
        ...
        else echo "<p>detik.usable ini sudah versi terbaru<p><form method=get action=$self><input type=hidden name=au value=1><input type=hidden name=commit value=1><input type=submit value=\"Force update again\"></form>";
