                ...
                while ($this->skipBlanksInTag()) {
                    ...
                    if ($attrName != "" && $attrName != "/") {
                        ...
                        if ($this->iCurrentChar == "=") {
                            ...
                            $this->na[strtolower($attrName)] = $value;
                        }
                        else {
                            $this->na[strtolower($attrName)] = "";
                        }
                    }
                }
        }
...
b            $this->skipMaxInTag ("\"", 1);
    ...
    function setTextIndex ($index) {
        ...
        if ($index >= $this->htl) {
            ...
            $this->iCurrentChar = $this->ht{$index};
        }
    }

    function moveNext() {
        if ($this->hti < $this->htl) {
            $this->setTextIndex ($this->hti + 1);
            return true;
        }
        else {
            return false;
    ...
    function skipToStringInTag ($needle) {
        $pos = strpos ($this->ht, $needle, $this->hti);
        if ($pos === false) {
            return "";
        }
        $top = $pos + strlen($needle);
        $retvalue = substr ($this->ht, $this->hti, $top - $this->hti);
