...
class DrawPanel extends JPanel {
	...
	public void paintComponent(Graphics g) {
		...
		/* Plot curve */
		g2.setClip(borderSize, borderSize, xSize-2*borderSize, ySize-2*borderSize);
		for (i = 0; i < npoints; i++) {
...
//			int minIdx = 0;
//			double minDist = Math.abs(currX - xScaled[0]);
//			double currDist = minDist;
//			for (i = 1; i < npoints; i++) {
//				currDist = Math.abs(currX - xScaled[i]);
//				if (currDist < minDist) {
//					minDist = currDist;
//					minIdx = i;
//				}
...
//			Line2D xLine = new Line2D.Double(borderSize, yScaled[minIdx], xSize
//					- borderSize, yScaled[minIdx]);
//			Line2D yLine = new Line2D.Double(xScaled[minIdx], ySize
//					- borderSize, xScaled[minIdx], borderSize);
//			g2.draw(xLine);
//			g2.draw(yLine);
//		}
		...
		this.drawXAxis(xSize, ySize, xScale, xMin, xMax, (int) axisLabelHeight,
				g2);
		this.drawYAxis(xSize, ySize, yScale, yMin, yMax, (int) axisLabelHeight,
				g2);
	...
	public void drawXAxis(int xSize, int ySize, double xScale, double xMin,
		...
		int numTicks = (int) ((double) (xSize - 2 * borderSize) / (double) textWidth);
		if (numTicks < 2)
			numTicks = 2;
		...
		double tickMin = d * Math.ceil(xMin / d);
		double tickMax = d * Math.floor(xMax / d);
	...
	public void drawYAxis(int xSize, int ySize, double yScale, double yMin,
		...
		// tick range should be smaller than the data range
		double tickMin = Math.ceil(yMin / d) * d;
		double tickMax = Math.floor(yMax / d) * d;
		...
		for (int i = 0; i < yTickPoints.size(); i++) {
			...
			double tickNumber = 0.;
			if (scientific) {
				tickNumber = (dp / multiplier); // Divide by exponent to get number
			} else {
				tickNumber = (dp);
			}
			tickLabel = tickFormatter.format(tickNumber);
	...
	private class GraphMouseHandler implements MouseListener,
			MouseMotionListener {
