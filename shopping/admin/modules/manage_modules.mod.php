...
class manage_modules extends TableManager {
    ...
    function manage_modules() {
        ...
        $this->grid_command[] = array('refresh_modules','RereadModules');
        $this->grid_command[] = array('ingen_csv',lang('CsvForInputComma'));
        $this->grid_command[] = array('enter_ingen_csv',lang('CsvForInputTab'));
        $this->grid_command[] = array('create_table',lang('CreateTable'));
        $this->grid_command[] = array('drop_table',lang('DropTable'));
        $this->grid_command[] = array('purge_table',lang('PurgeTable'));
    }
    ...
    function act_create_table($post) {
        ...
        foreach ($prog->properties as $key=>$col) {
            if ($col->colname == '') continue; # skip if colname is not exists
    ...
    function create_random_text($length) {
        $txt = 'abcd efghi jklmn opqrs tuvwx yzABC DEFGH IJKLM NOPQR STUVW XYZ';
        $txt_size = strlen($txt)-1;
        $result = '';
        for ($i=0;$i<$length;$i++) {
            $result .= $txt[rand(0,$txt_size)];
        }
        return $result;
    }

    function act_auto_test_data($post) {
    /*
    purpose: insert 50 random data test, depending on datatype
    */
        if ($post) {
            $mdl = instantiate_module($_REQUEST['m2']);
            $mdl->clear();
            foreach ($mdl->properties as $colvar=>$col) {
                for ($i=0;$i < $_REQUEST['test_num']; $i++) {
                    if (!$col->required and rand(0,1) == 1)
                        continue;
                    # fix default length
                    if ($col->length == 255)
                        $col->length = 30;
                    ...
                    if (0) {}
                    elseif ($col->enumerate != '' and $col->datatype == 'int') {
                        #foreign key - get a random rowid from target table
                        $mdl2 = instantiate_module($col->enumerate);
                        $rows = $mdl->get_rows(array(),'rowid','row',$mdl2->db_table);
                        $value = $rows[rand(0,count($rows)-1)][0];
                    }
                    elseif ($col->datatype == 'int') {
                        # create random number between 0-65000
                        $value = rand(0,65000);
                    }
                    elseif ($col->datatype == 'varchar') {
                        # create random string 0 - length char
                        $length = rand(1,$col->length);
                        $value = $this->create_random_text($length);
                    }
                    elseif ($col->datatype == 'date') {
                        $secs = rand(0,50000000);
                        $random_time = time() + $secs - 25000000;
                        $value = date('Y-m-d', $random_time);
                    }
                    elseif ($col->datatype == 'datetime') {
                        $secs = rand(0,50000000);
                        $random_time = time() + $secs - 25000000;
                        $value = date('Y-m-d H:i:s', $random_time);
                    }
                    elseif ($col->datatype == 'float') {
                        $d1 = rand(-50000000,50000000);
                        $d2 = rand(-50000000,50000000);
                        $value = $d1+(1/$d2);
                    }
                    ...
                    elseif ($col->datatype == 'blob') {
                        # not handled yet
                    }
                    $mdl->ds->{$colvar}[$i] = $value;
                }
            }
            $mdl->db_count = $_REQUEST['test_num'];
            $mdl->action = 'new';
            $mdl->showgrid();
        ...
        if ($_REQUEST['test_num'] == '') $_REQUEST['test_num'] = 20;

        echo $this->__message;
        echo '<p>Specify number of test data to insert:';
        echo '<form method=post action="'.$_SERVER['PHP_SELF'].'">';
        echo '<input type=hidden name=m value="'.$this->module.'">';   # this module
        echo '<input type=hidden name=m2 value="'.$_REQUEST['m2'].'">';
        echo '<input type=hidden name=act value="auto_test_data">';   # contains the action (edit/new)
        echo '<input type=hidden name=save value="1">';         # marker to indicate form submitation
        echo '<input type=text name=test_num value="'.$_REQUEST['test_num'].'">';         # marker to indicate form submitation
    ...
    function act_enter_ingen_csv($post) {
        if ($post) {
            ...
            fclose($fp);
            if (count($rows) < 3) {
                $this->__message = '<p><b>Warning: Inserted CSV Row is less than 3!! You must paste WITH the first 2 row intact, as given by Generate CSV for Input.';
                return;
            }
