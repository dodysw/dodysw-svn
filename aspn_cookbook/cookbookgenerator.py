...
query_limit = 900 # aspn index end, increase this if you see that aspn cookbook collection has exceeded 900. you can safely put 5000 if you like, but it gets slower since i'd download until index 5000 without getting new articles.
...
try: os.removedirs(abs_path + 'result')    # clean result folder
except: pass
try: os.mkdir(abs_path + 'cache')
except: pass
try: os.mkdir(abs_path + 'cache/index')    # contain aspn indices caches
except: pass
try: os.mkdir(abs_path + 'cache/page') # contain aspn page caches
except: pass
try: os.mkdir(abs_path + 'result')
except: pass
...
while query_start < query_limit:
    ...
    try: # to get from cache
        cache_idxfile = abs_path + 'cache/index/idx%s.html' % query_start
...
for i,urld in enumerate(queue_downloads):
    ...
    if os.access(abs_path + 'cache/page/'+filename,os.F_OK): # check file existance
        print 'exist'
        continue
    buffer = urllib2.urlopen(urld).read()   # download cookbook
    print 'ok'
    file(abs_path + 'cache/page/' + filename,'wb').write(buffer)   # save to cache file
...
tmpl = file(abs_path + 'include/template.tpl','rb').read()
tmpl_cat = file(abs_path + 'include/templatecat.tpl','rb').read()
tmpl_main = file(abs_path + 'include/templatemain.tpl','rb').read()
tmpl_toc = file(abs_path + 'include/templatetoc.tpl','rb').read()
...
categories = {}
for filename in glob.glob(abs_path + 'cache/page/*.html'):
    ...
    file(abs_path + "result/" + os.path.basename(filename),'wb').write(newfile)  # then create new pages based on template in template.tpl using those informations
...
for cat in catsort:
    ...
    file(abs_path + 'result/%s.html' % cat,'wb').write(newfile)  # category index: commit to file
toc += '</UL>\n'

# then, create the table of content
newfile = tmpl_toc % dict(toc=toc)    # table of content : paste variables into template
file(abs_path + "result/toc.hhc",'wb').write(newfile)    # table of content: commit to file
...
file(abs_path + "result/main.html",'wb').write(newfile)  # main page: commit to file

# create hpp file, based on template.hhp so that i can automate modified-date
buffer = file(abs_path + 'include/template.hhp','rb').read()
buffer = buffer % dict(
    title=hhp_title,
    compiled_file = abs_path + 'aspnpython-%s.chm' % datetime.date.today().isoformat()
    )
file(abs_path + "result/cookbook.hhp",'wb').write(buffer)    # table of content: commit to file

# copy additional files
shutil.copy(abs_path + 'include/aspn.css',abs_path + 'result/')

# spawn html workshop, if defined
if html_workshop_path:
