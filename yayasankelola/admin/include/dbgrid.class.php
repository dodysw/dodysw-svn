...
function submit_confirm(myform) {
    ...
    return false;
        ...
        $_submitlabel = ($this->preview[$this->action] and !$this->_preview)? ' '.lang('Preview').' ': ' '.lang('Save').' ';
...
/* ==== Overrideable functions */
    ...
    function check_datatype($i) {
        ...
        foreach ($this->properties as $colvar=>$col) {
            if ($this->ds->{$colvar}[$i] != '' and ($col->datatype == 'double' or $col->datatype == 'int' or $col->datatype == 'float') and !is_numeric($this->ds->{$colvar}[$i])) {
                $label = trim($col->label) == ''? $colvar: $col->label;
                $this->error_msgs[] = '['.($i+1).'] '.$label.' only accept numeric data (You supply ['.$this->ds->{$colvar}[$i].'])';
                $this->error_rows[$i] = True;
                return False;
            }
    ...
    function insert($rowindex) {
        #~ echo 'Magic quote:'.get_magic_quotes_gpc();
        ...
        foreach ($this->properties as $colvar=>$col) {
            ...
            if ($value == 'Now()') {
                ...
                #~ echo '<br>Value1:'.$value.' Value2:'.myaddslashes($value);
                $tv[] = "'".myaddslashes($value)."'";
            }
        }
        $sql_fields = join(',',$tf);
