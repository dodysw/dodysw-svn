...
class dirstruct extends TableManager {
    ...
    function front_list($cat_id = '', $link_edit = False) {
        ...
        $col_num = 2;
        echo '<table border=0 cellspacing=20 cellpadding=0>';
    ...
    function front_entry($cat_id,$link_edit=False) {
        ...
        for ($i = 0; $i < $entry->db_count; $i++) {
            ...
            echo '<p><b><a href="'.get_fullpath().$entry_module.'.php?'.merge_query(array('eid'=>$entry->ds->entry_id[$i])).'">'.$entry->ds->org_nama[$i].'</a></b>';
        }
