...
class linkstruct extends TableManager {
    var $db_table, $properties;
    function linkstruct() {
        parent::TableManager(); # must call base class

        global $html_title;
        $this->title = lang('Link');
        $html_title = $this->title;

        $this->db_table = $GLOBALS['dbpre'].'linkstruct_tab';
        ...
        $this->properties['parent_link'] = new Prop(array('label'=>lang('Parent'),'colname'=>'parent_link', 'required'=>True, 'enumerate'=>'linkstruct'));
        $this->enum_keyval = array('id','name');
        $this->unit = 'link category';
    ...
    function act_browse_structure($post) {
        $this->front_list($_REQUEST['lpa'],True);
    }

    function prepare_insert($i) {
        $seq = instantiate_module('seq_gen');
        $this->ds->id[$i] = $seq->simulate_next_number('link_id');
        return True;
    }

    function insert($i) {
        $seq = instantiate_module('seq_gen');
        $this->ds->id[$i] = $seq->get_next_number('link_id');
        parent::insert($i);
    }

    function front_list($cat_id = '', $link_edit = False) {
        $parent_field = 'parent_link';
        $url_key = 'lpa';
        ...
        if ($cat_id != '*') {
/* no need to display name and description
...
*/
        ...
        for ($i = 0; $i < $this->db_count; $i++) {
            ...
            elseif ($i % $col_num == ($i-1)) {  # right part
                echo '<td valign="top">';
            }
            else {  # middle part
                echo '<td valign="top">';
            ...
            echo '<span style="font-size:10"><b><a href="'.$_SERVER['PHP_SELF'].'?'.merge_query(array($url_key=>$this->ds->id[$i])).'">'.$this->ds->name[$i].'</a></b></span>';
    ...
    function front_entry($cat_id,$link_edit=False) {
        ...
        for ($i = 0; $i < $entry->db_count; $i++) {
            ...
            echo '<p><b><a href="'.$entry->ds->url[$i].'">'.$entry->ds->name[$i].'</a></b>: '.$entry->ds->description[$i];
        }
