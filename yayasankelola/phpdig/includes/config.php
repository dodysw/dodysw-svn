...
#~ if ( (!isset($relative_script_path) ) ||
    #~ ( ($relative_script_path != ".") && ($relative_script_path != "..") && ($relative_script_path != ABSOLUTE_SCRIPT_PATH))
    #~ ) {
  #~ echo "\n\nPath not recognized!\n\n";
  #~ exit();
#~ }
...
define('PHPDIG_VERSION','1.8.3');

$phpdig_language = "id"; // cs, da, de, en, es, fr, gr, it, nl, no, pt
...
define('PHPDIG_ADM_PASS','admin123'); //Password

//template file and style
if (isset($_GET['template_demo'])) { $template_demo = $_GET['template_demo']; }
$templates_array = array('black.html','bluegrey.html','corporate.html','green.html','grey.html','lightgreen.html','linear.html','newspaper.html','phpdig.html','simple.html','terminal.html','yellow.html');
...
//$template = "$relative_script_path/templates/phpdig.html"; // if using array set $template = "array";
...
if ((!isset($template)) || ((!is_file($template)) && ($template != "array"))) {
