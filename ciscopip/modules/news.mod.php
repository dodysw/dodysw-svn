...
class news extends TableManager {
    ...
    function news() {
        ...
        $this->properties['body'] = new Prop(array('label'=>'Body','colname'=>'body','datatype'=>'text','inputtype'=>'textarea','inputtype2'=>'htmlarea','rows'=>10,'browse_maxchar'=>30));
        ...
        #~ $this->properties['create_date_date'] = new Prop(array('label'=>'Creation Date','colname'=>"date_format(create_date,'%d %M %Y')",'updatable'=>False,'insertable'=>False));
    ...
    function fe_list($maxrow,$cat_id='*') {
        ...
        #~ $cat = instantiate_module('category');
        for ($i = 0; $i < $this->db_count; $i++) {
            ...
            #~ echo "<p><b><a href='{$_SERVER['PHP_SELF']}?m=news&id={$this->ds->_rowid[$i]}'>{$this->ds->title[$i]}</a></b> <small>({$this->ds->author[$i]} - {$this->ds->create_date[$i]})</small> ";
            $create_date = date('d M Y',dbdatetime2unix($this->ds->create_date[$i]));
            echo "<p><b><a href='{$_SERVER['PHP_SELF']}?m=news&id={$this->ds->_rowid[$i]}'>{$this->ds->title[$i]}</a></b> <small>({$create_date})</small> ";
        ...
        */
        ...
        if ($year != '' and $month != '') {
            ...
            foreach ($yearly[$year][$month] as $i) {
                $catdesc = $cat->enum_decode($this->ds->category[$i]);
                echo '<li>';
                echo "<a href='news.php?id={$this->ds->_rowid[$i]}'>{$this->ds->title[$i]}</a> <small>({$this->ds->author[$i]} - {$this->ds->create_date[$i]} - $catdesc)</small>";
                echo '</li>';
            }
            echo '</ul>';
        }
    ...
    function construct_view($rowid) {
        ...
        #~ $buffer .= "<p><b>{$this->ds->title[$i]}</b> <small>({$this->ds->author[$i]} - {$this->ds->create_date[$i]})</small> ";
        $create_date = date('d M Y',dbdatetime2unix($this->ds->create_date[$i]));
        $buffer .= "<p><b>{$this->ds->title[$i]}</b> <small>($create_date)</small> ";
    ...
    function construct_list_email($rowid) {
        ...
        for ($i=0;$i<$this->db_count;$i++) {
            ...
            $buffer .= "<a href='".get_fullpath()."news.php?id={$this->ds->_rowid[$i]}'>{$this->ds->title[$i]}</a> <small>({$this->ds->author[$i]} - {$this->ds->create_date[$i]})</small>";
    ...
    function shownewrecord() {
        if ($this->allow_new) {
            ...
            echo '<input type=hidden name="num_row" value="1"> <input type=submit value="Add News"></p>';
            echo '</form>';
        }
    }
