...
/*
    ...
    dody suria wijaya's software house - http://dsw.gesit.com
...
//var $logger;
    function TableManager() {
        ...
        $this->allow_query = False; # bool, True to show query form
        ...
        $this->browse_mode_forced = True;   # bool, True, will not give options for user to change browse mode
        $this->browse_form_cols = 1;   # int, the number of columns in browse-form mode
        $this->browse_rows = 0; # int, rows displayed per page. 0 for all rows.
        ...
        $this->submit_label['new'] = ' Save ';  # label for submit button at 'new' action
        $this->submit_label['edit'] = ' Save ';  # label for submit button at 'edit' action
        ...
        $this->grid_no_headersort = True;   #bool, True to disable grid header's sort function
        ...
        #~ $this->logger->debug('Starting '.$this->module.' - '.$_SERVER['REQUEST_METHOD']);
    }
    ...
    function final_init() {
        ...
        */
		...
		foreach ($this->properties as $key=>$col) {
			#~ $this->logger->debug('variable key ' . $key );
        ...
        if ($this->module == $_REQUEST['m'] and $_SERVER['REQUEST_METHOD'] == 'POST') {    # only do this if I'm the currently active module in page
			#~ $this->logger->debug('panggil POST HANDLER ,......................');
            $this->post_handler();
        }
        #~ $this->logger->debug('end method final_init.....144' );

    }

    function post_handler() {
        /* called aumatically by final_init to provide post request handling*/
		#~ $this->logger->debug('Starting post_handler......152');
        ...
        if ($this->action == 'new' or $this->action == 'edit') { # handle new-save
            ...
            if ($this->_save == '') {   # to accomodate -1 (preview)
				#~ $this->logger->debug('nilai $this->_save tidak sama dengan 1');
				return False;
			}
            ...
            #~ $this->logger->debug('pass basic row validation');

            if ($this->action == 'new') {
				#~ $this->logger->debug('action adalah 177 ' .$this->action);
                #~ $this->import2ds($row); # import _REQUEST var to ds
                ...
                #~ $this->logger->debug('pass check_insert');
                ...
                #~ $this->logger->debug('pass preview mode branch');
                ...
                if ($this->_go != '') {
					...
					#~ $this->logger->debug('212. Going to ' .$this->_go);
        ...
        elseif ($this->action == 'csv') {
            ...
            */
            ...
            #~ }
				...
				#~ $this->logger->debug('end post_handler......257');
    }
    ...
    function basic_handler() {
        ...
        */
        ...
        if ($this->action == 'new' or $this->action == 'duplicate') {
            ...
            if ($this->db_count == 1) {
				#~ $this->logger->debug('Masuk ke Showform......279');
				$this->showform();

            }
            else {
                # prepare as many as rownum
                $this->showgrid();
            }
        }
        elseif ($this->action == 'del') {
            ...
            echo '<p>'.$this->db_count.' row(s) has been deleted</p>';
        ...
        elseif ($this->action == 'browse') {
            ...
            if (method_exists($this, $callback))
                ...
                die('Unhandled action "'.$this->action.'". Stopping...');
        }
        ...
        if (($this->action == 'browse' and !$this->browse_wait_query) or ($this->action == 'browse' and $this->browse_wait_query and $this->_query != '')) {
            ...
            if (!$this->browse_mode_forced and $_SESSION['module_browse_mode'][$this->module] != '') { # last browsemode of given module
				...
				#~ $this->logger->debug('end basic_handler......364');
    }
...
#----- helper functions ----- #
    ...
    function validate_field($colvar,$i) {
        ...
        if  (
            ...
            $this->error_msgs[] = "[".($i+1)."] {$col->label} is required";
...
#----- end of helper functions ----- #
    ...
    function import2ds() {
        ...
        */
        #~ foreach ($_REQUEST['field'] as $key=>$val) { # try blind copy (might pose security hazard)
        #~ debug();
		#~ $this->logger->debug('starting function import2ds...606' );
...
/* presentation functions */
    function showview() {
        ...
        echo '<p><b><a href="" onclick="window.history.back();return false;">Cancel</a></b>';
    ...
    function showquery() {
        ...
        echo "<option value='*'>All fields</option>";
    ...
    function showgrid($action='') {
        ...
        #~ $this->grid_command[] = array('csv','Export CSV');
        ...
        if ($this->action == 'browse') {
            echo 'function submit_confirm(myform) {
        ...
        # create table view
        echo '<form method=post action="'.$_SERVER['PHP_SELF'].'" onSubmit="return submit_confirm(this)">';
        echo '<input type=hidden name=m value="'.$this->module.'">';
        if ($action == 'browse') {
        ...
        echo '<th class="heading"><h3>'.$this->title.'</h3>';
        #~ echo '<td align="right">';
        #~ if ($this->allow_new) {
        ...
        #~ }
        echo '</th>';
        # custom: this will put query next to grid title
        if ($this->allow_query and $action == 'browse' and !$this->logical_parent) {
            echo '<th>'; $this->showquery(); echo '</th>';
        }
        if (!$this->browse_mode_forced)
            echo "<td><small>[<a href='{$_SERVER['PHP_SELF']}?m={$this->module}&set_browse_mode=form'>switch to form</a>]</small></td>";
        ...
        if ($this->db_count > 0 or $_REQUEST['row'] != '') {
            ...
            echo '</table>';
            echo '<p align="center"><b>No records found</b>';
        }

        echo $this->body[$this->action]['suffix'];  # show prefix/suffix body

        $_submitlabel = ($this->preview[$this->action] and !$this->_preview)? ' Preview ': ' Save ';
        ...
        if ($action == 'browse' and $this->db_count > 0 and !$this->query_only and $this->grid_command) {
            ...
            echo '<input type="button" onclick="setCheckBoxes(this.form, true);" value="Check All"> / ';
            # echo '<a href="'.$_SERVER['PHP_SELF'].'" onclick="setCheckBoxes(this.form,false);return false;">Uncheck All</a> ';
            echo '<input type="button" onclick="setCheckBoxes(this.form, false);" value="Uncheck All"> ';
    ...
    function shownewrecord() {
        if ($this->allow_new) {
				#~ $this->logger->debug('show new record '  );
    ...
    function showform() {
        ...
        if ($this->allow_new) {
            $this->grid_command[] = array('new','New');  # command, need to be list of list, since key may be duplicated
        }
        ...
        #~ $this->grid_command[] = array('csv','Generate CSV');
        ...
        echo '<tr valign="top">';
        echo '<th class="heading"><span class="contenttitlealt"><h3>'.ucfirst($this->action).": {$this->title}</h3></span></th>";
        ...
        if ($this->action == 'browse') {
            ...
            if ($this->grid_command) {
                ...
                echo '<option value="">__Action__</option>';
                foreach ($this->grid_command as $command) {
                    echo "<option value='{$command[0]}'>{$command[1]}</option>";
                }
                echo '</select>';
            ...
            #~ }
            # custom: show query next to form title
            if ($this->allow_query and $this->action == 'browse' and !$this->logical_parent) {
                echo '<th>'; $this->showquery(); echo '</th>';
            }
        ...
        if (!$this->browse_mode_forced)
            echo "<td>&nbsp;&nbsp;<small>[<a href='{$_SERVER['PHP_SELF']}?m={$this->module}&set_browse_mode=table'>switch to table</a>]</small></td>";
        echo '</tr></table>';  //outer table

        if ($this->_query != '') {
            if ($this->db_count == 0) {
                echo '<p><b>Search "'.$this->_query.'" returns no record</b>. <a href="'.$_SERVER['PHP_SELF'].'?m='.$this->module.'">Reset Search</a>.</p>';
                return;
            }
            else {
                echo '<p><b>Search "'.$this->_query.'" found '.$this->db_count.' record(s)</b>. <a href="'.$_SERVER['PHP_SELF'].'?m='.$this->module.'">Reset Search</a>.</p>';
            }
        }
        ...
        if ($this->action != 'browse')
            echo '<p><span class="asterix">*</span>Indicates required fields';

        echo '<table border="0">';

        echo '<form method=post action="'.$_SERVER['PHP_SELF'].'" onSubmit="return form_submit_confirm(this);">';
        ...
        for ($ci = 0; $ci < count($this->colgrid); $ci++) {
            ...
            if ($col->box_start != '') {    # box start append a title line
                ...
                # custom: add a line before box start
                echo '<tr><td colspan="2"><br>
                <table border="0" cellpadding="0" cellspacing="0" width="100%">
                    <tr><td><img src="/swa/i/s.gif" width="1" height="3" alt=""></td></tr>
                    <tr><td bgcolor="#999999"><img src="/swa/i/s.gif" width="1" height="3" alt=""></td></tr>
                </table>
                <SPAN class="contenttitlealt">'.$col->box_start.'</SPAN></td></tr>';
            ...
            for ($colspan=0; $colspan < $max_colspan; $colspan++) {
                ...
                elseif ($this->_preview) {   # preview me
                    ...
                    if ($col->custom_isphonearea and $this->ds->{$colvar}[$this->_cursor] != '')
                        echo '('.$this->ds->{$colvar}[$this->_cursor].')';
                    elseif ($col->custom_isext and $this->ds->{$colvar}[$this->_cursor] != '')
                        echo 'Ext-'.$this->ds->{$colvar}[$this->_cursor];
                    else
                        echo ' '.$this->ds->{$colvar}[$this->_cursor].' ';
                }
                else {
            ...
            if ($this->action != 'browse' and ($this->action == 'edit' or $this->action == 'new')) {   # edit/add mode and not preview
                if (!$col->custom_previewhidenotes)
                    echo '&nbsp;'.$col->notes;
            }
            echo '</td>';
    ...
    function show_childgrid() {
        ...
        if (($this->action=='edit' or $this->action=='view' or $this->action=='browse') and $this->childds) {
            foreach ($this->childds as $child_modulename) {
                include_once(APP_MODULE_ROOT .'/'. $child_modulename.'.mod.php');
                $child_module = new $child_modulename();
        ...
        switch ($col->inputtype) {
            ...
            case 'checkbox':
                if ($col->enumerate == '') {    # case 1, no enumerate, a boolean checkbox
                    ...
                    $enum_list = array();
                    if (is_string($col->enumerate)) {
                        include_once($col->enumerate.'.mod.php');
                        $e = new $col->enumerate();
            ...
            case 'textarea':
                ...
                if ($col->inputtype2 == 'htmlarea' and $this->action != 'browse') {
                    ...
                    $html_footer_strings .= '<script>HTMLArea.replace("'.$fieldname.'");</script>';
                }
            ...
            case 'combobox':
                ...
                foreach ($enum_list as $id=>$text) {
                    $value == $id? $ischecked = 'selected': $ischecked = '';
...
/* ==== Overrideable functions */
    ...
    function XXXdelrow() {
        ...
        if ($rowid == '') {
            global $last_message;
            $last_message = '<p>You need to pick at least one row</p>';
            $this->action = 'browse';
            return;
        }
        # now uses javascript confirmation
        # directly delete row
        $rowids = join(', ',$rowid);
        $sql = "delete from {$this->db_table} where rowid in ($rowids)";
        $res = mysql_query($sql) or die('<br>'.$sql.'<br>'.mysql_error());
        echo '<p>'.count($rowid).' row(s) has been deleted</p>';
        echo '<p><b><a href="'.$this->_go.'">Continue</a></b></p>';
        return;
    }
